...
- 데모 계정 발급 후 CRM 대시보드 진입 시 상단 바, KPI 카드 및 프로필 메뉴에 `demo_id`가 할당되어 관리됩니다.

### 2. company_id 생성 시점
- 데모 CRM 내부에서 `[위시드 CRM 시작하기]` 또는 프로필 드롭다운의 `[결제하기]` 클릭 후 **Step 03 회원사 정보 입력** 완료 시점에 `company_id` (예: `COMP-2026-9901`)가 생성됩니다.
